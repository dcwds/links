...
const postDatabaseSchema = async () => {
  ...
  if (!process.env.FAUNADB_LOCAL_SECRET) {
    console.log(
      `
        You need to configure a FAUNADB_LOCAL_SECRET environment variable.
  ...
  try {
    const response = await fetch(endpoint, {
      method: "POST",
      headers: {
        Authorization: `Bearer ${process.env.FAUNADB_LOCAL_SECRET}`,
